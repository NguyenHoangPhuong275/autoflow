...
const DraggableNode: React.FC<DraggableNodeProps> = ({ nodeRef, containerRef, onDragMove, className, children }) => {
    const { onPointerDown, style } = useDraggable(nodeRef, containerRef, onDragMove);
    return (
        <div
            ref={nodeRef}
            className={className}
...
export const PipelineVisualizer: React.FC<PipelineVisualizerProps> = ({ activeSourceId, stage, }) => {
    ...
    return (
        <div ref={containerRef} className="relative flex h-full w-full items-center justify-between overflow-hidden whitespace-nowrap rounded-lg p-2.5 font-mono text-xs panel-card">
          ...
          {/* Source nodes — draggable */}
          <div className="z-10 flex flex-col gap-1 pt-4">
            {sources.map(({ id, label, icon: Icon, ref, activeClass }) => (
              <DraggableNode
                key={id}
                nodeRef={ref as React.RefObject<HTMLDivElement>}
                containerRef={containerRef as React.RefObject<HTMLDivElement>}
                onDragMove={handleDragMove}
                className={`flex items-center gap-1 rounded border px-1.5 py-0.5 text-[10px] transition-colors ${activeSourceId === id
          ...
          {/* Engine node — draggable */}
          <DraggableNode
            nodeRef={engineRef as React.RefObject<HTMLDivElement>}
            containerRef={containerRef as React.RefObject<HTMLDivElement>}
            onDragMove={handleDragMove}
            className={`z-10 mt-4 flex flex-col items-center gap-1 rounded-lg border px-3 py-1.5 text-center transition-colors ${isRunning
          ...
          {/* DeepSeek node — draggable */}
          <DraggableNode
            nodeRef={deepSeekRef as React.RefObject<HTMLDivElement>}
            containerRef={containerRef as React.RefObject<HTMLDivElement>}
            onDragMove={handleDragMove}
            className={`z-10 mt-4 flex items-center gap-1.5 rounded border px-2 py-1.5 text-[10px] font-bold ${isRunning
