...
export const TerminalLogs: React.FC<TerminalLogsProps> = ({ logs, onClearLogs }) => {
    ...
    const { copied, copy, copyError } = useClipboardFeedback(1500);
    ...
    const handleCopy = () => {
        const text = logs.map((l) => `${l.timestamp} · ${getLevelLabel(l.level)}: ${l.message}`).join('\n');
        copy(text);
    };
    const getTag = (level: LogLevel) => {
        switch (level) {
            case 'process': return <span className="text-purple-400 font-bold shrink-0">ĐANG XỬ LÝ</span>;
            case 'success': return <span className="text-emerald-400 font-bold shrink-0">THÀNH CÔNG</span>;
            case 'warn': return <span className="text-amber-400 font-bold shrink-0">CẢNH BÁO</span>;
            case 'error': return <span className="text-rose-400 font-bold shrink-0">THẤT BẠI</span>;
            default: return <span className="text-cyan-400 font-bold shrink-0">THÔNG TIN</span>;
        }
    };
    return (<div className="panel-card rounded-lg flex flex-col h-full overflow-hidden text-xs font-mono">
      ...
      {copyError && <div role="alert" className="border-b border-rose-500/30 bg-rose-950/40 px-3 py-2 text-[10px] text-rose-300">{copyError}</div>}
