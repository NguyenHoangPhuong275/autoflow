...
export const AiCopilotChat: React.FC<AiCopilotChatProps> = ({
  ...
  const {
    ...
    toggleDocPermission,
    handleAddCustomDoc,
  } = useAgentDocuments(externalDocuments);
  ...
  const handleUndo = async () => {
    ...
    try {
      ...
      const message = getUserErrorMessage(error, 'Vui lòng thử lại thao tác hoàn tác.');
      const errorMsg: ChatMessage = {
        id: `undo-err-${Date.now()}`,
        sender: 'system',
        text: `Không thể hoàn tác thao tác vừa chọn. ${message}`,
        timestamp: new Date().toLocaleTimeString('vi-VN', { hour12: false }),
      };
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const message = getUserErrorMessage(error, 'Vui lòng kiểm tra dữ liệu và thử lại.');
      const errorMsg: ChatMessage = {
        id: `err-${Date.now()}`,
        sender: 'system',
        text: `Không thể xử lý yêu cầu hiện tại. ${message}`,
        timestamp: new Date().toLocaleTimeString('vi-VN', { hour12: false }),
      };
      setMessages((prev) => [...prev, errorMsg]);
    } finally {
  ...
  return (
    <div className="panel-card rounded-lg flex flex-col h-full overflow-hidden text-xs font-mono">
      ...
      <div className="px-2.5 py-1 bg-[#070a12] border-b border-[#1a2336] flex items-center gap-1.5 overflow-x-auto text-[10px] shrink-0">
        <span className="text-slate-500 font-bold uppercase shrink-0">Nguồn dữ liệu:</span>
        <span className="px-2 py-0.5 rounded flex items-center gap-1 shrink-0 bg-[#0e1c2e] border border-emerald-500/40 text-emerald-300 font-medium">
          ...
          <span>Sheet: {activeSheetTitle || 'chưa chọn'}</span>
        </span>
      ...
      <div className="flex-1 p-3 bg-[#070a12] overflow-y-auto space-y-3 select-text">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-2 text-[11px] leading-relaxed ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.sender !== 'user' && (
              <div
                className={`w-6 h-6 rounded border flex items-center justify-center shrink-0 mt-0.5 ${
                  msg.sender === 'system'
                    ? 'bg-rose-950/80 border-rose-800 text-rose-300'
                    : 'bg-[#162036] border-[#232f48] text-cyan-400'
                }`}
              >
                {msg.sender === 'system' ? <CircleAlert className="w-3.5 h-3.5" /> : <Bot className="w-3.5 h-3.5" />}
              </div>
            )}
            ...
            >
              ...
              {msg.options && msg.options.length > 0 && (
                <div className="mt-2.5 pt-2 border-t border-[#1a2336] space-y-1.5">
                  ...
                  <div className="grid grid-cols-1 gap-1.5">
                    {msg.options.map((opt, optIdx) => (
                      <button
                        ...
                        disabled={isLoading}
                        className="w-full text-left p-2 rounded-md bg-[#090e1a] hover:bg-[#152136] border border-[#1e2a42] hover:border-indigo-500/80 transition-all flex items-center justify-between group disabled:opacity-50 active:scale-[0.99] shadow-sm"
                      >
                        <div className="flex items-center gap-2 min-w-0 pr-2">
                          {opt.previewBg ? (
                            <div
                              className="w-4 h-4 rounded shrink-0 border border-white/20 shadow-sm flex items-center justify-center font-bold text-[8px]"
                              style={{
                            ...
                            <div className="w-4 h-4 rounded shrink-0 bg-indigo-950 border border-indigo-700/50 flex items-center justify-center text-indigo-300 font-bold text-[9px]">
                              {optIdx + 1}
                            </div>
                          )}
                          <div className="min-w-0">
                            <div className="font-bold text-slate-200 group-hover:text-white text-[11px] truncate flex items-center gap-1.5">
                              ...
                              {opt.badge && (
                                <span className="px-1 py-0.2 rounded bg-indigo-950/80 text-indigo-300 text-[9px] border border-indigo-700/50">
              ...
              {msg.actionSummary && (
                <div className="mt-1 flex items-center justify-between gap-2 px-2 py-0.5 rounded bg-emerald-950/70 border border-emerald-700/60 text-emerald-300 text-[10px]">
            ...
            {msg.sender === 'user' && (
              <div className="w-6 h-6 rounded bg-indigo-700 flex items-center justify-center text-white shrink-0 mt-0.5">
      ...
      <div className="px-2.5 py-1 bg-[#090d16] border-t border-[#1a2336] flex items-center gap-1.5 overflow-x-auto text-[10px] shrink-0">
        {QUICK_ACTIONS.map(({ label, prompt, icon: Icon }) => (
          <button
            ...
            disabled={isLoading}
            className="px-2 py-0.5 rounded bg-[#111726] hover:bg-[#162036] border border-[#1a2336] text-slate-400 hover:text-slate-200 shrink-0 disabled:opacity-40 transition-colors flex items-center gap-1.5"
          >
      ...
      <div className="p-2 bg-[#0b101c] border-t border-[#1a2336] flex items-center gap-1.5 shrink-0">
        <input
          ...
          placeholder="Hỏi hoặc ra lệnh theo dữ liệu bạn đang quản lý..."
          className="flex-1 bg-[#070a12] border border-[#1a2336] rounded-md px-2.5 py-1.5 text-[11px] text-[var(--text-primary)] placeholder-slate-600 focus:outline-none focus:border-indigo-500 font-sans"
        />
        <button
          onClick={() => handleSendMessage()}
          disabled={isLoading || !input.trim()}
          className="p-1.5 rounded-md bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-40 transition-colors shrink-0"
        >
      ...
      {showDocModal && (
        <div
          ...
          aria-modal="true"
        >
          <div className="w-full max-w-md bg-[#0c121e]/95 border border-slate-700/60 rounded-2xl p-5 space-y-4 text-slate-100 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] backdrop-blur-xl">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-3 font-bold text-sm sm:text-base">
              <div className="flex items-center gap-2 text-cyan-400">
                <div className="p-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/30">
                  <FileText className="w-4 h-4" />
                </div>
                <span>Cấp Quyền Tài Liệu Cho AI</span>
              </div>
              <button
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1.5">
                  Tên tài liệu / Tiêu đề ngữ cảnh:
                </label>
                <input
                  type="text"
                  value={newDocName}
                  onChange={(e) => setNewDocName(e.target.value)}
                  placeholder="Ví dụ: Bảng giá ưu đãi tháng 8, Quy định vận chuyển..."
                  className="w-full bg-[#111928] border border-slate-700/60 rounded-xl px-3 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 font-sans"
                />
              </div>

              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1.5">
                  Nội dung văn bản / Quy tắc bổ sung:
                </label>
                <textarea
                  value={newDocContent}
                  onChange={(e) => setNewDocContent(e.target.value)}
                  rows={4}
                  placeholder="Dán nội dung văn bản, ghi chú hoặc quy tắc nghiệp vụ tại đây để AI đọc và tham khảo..."
                  className="w-full bg-[#111928] border border-slate-700/60 rounded-xl p-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 resize-none leading-relaxed font-sans"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2.5 pt-3 border-t border-slate-800/80">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-xs disabled:opacity-40 transition-all shadow-md shadow-cyan-950/40"
              >
                Cấp Quyền Tài Liệu
              </button>
            </div>
